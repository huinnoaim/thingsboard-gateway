...
MQTT_VERSIONS = {3: MQTTv31, 4: MQTTv311, 5: MQTTv5}
...
class MqttConnector(Connector, Thread):
    def __init__(self, gateway, config, connector_type):
        ...
        self.__send_data_only_on_change = self.__broker.get(
            SEND_ON_CHANGE_PARAMETER, DEFAULT_SEND_ON_CHANGE_VALUE
        )
        self.__send_data_only_on_change_ttl = self.__broker.get(
        ...
        self.load_handlers(
            "serverSideRpc", mandatory_keys["serverSideRpc"], self.__server_side_rpc
        )
        ...
        self.__max_msg_number_for_worker = config["broker"].get(
            "maxMessageNumberPerWorker", 10
        )
        self.__max_number_of_workers = config["broker"].get("maxNumberOfWorkers", 100)

        self._on_message_queue = Queue()
        self._on_message_thread = Thread(
            name="On Message", target=self._process_on_message, daemon=True
        )
    ...
    def load_handlers(self, handler_flavor, mandatory_keys, accepted_handlers_list):
        if handler_flavor not in self.config:
                ...
                for key in mandatory_keys:
                    if key not in handler:
                        ...
                        self.__log.error(
                            ...
                            key,
                            handler_flavor,
                            simplejson.dumps(handler),
                        )
                    else:
                        self.__log.debug(
                            "Mandatory key '%s' found in %s handler: %s",
                            key,
                            handler_flavor,
                            simplejson.dumps(handler),
                        )
            ...
            self.__log.info(
                "Number of accepted %s handlers: %d",
                handler_flavor,
                len(accepted_handlers_list),
            )

            self.__log.info(
                "Number of rejected %s handlers: %d",
                handler_flavor,
        ...
        while not self._connected and not self.__stopped:
            try:
                self._client.connect(
                    self.__broker["host"], self.__broker.get("port", 1883)
                )
    ...
    def _on_connect(self, client, userdata, flags, result_code, *extra_params):
        if result_code == 0:
            ...
            self.__log.info(
                "%s connected to %s:%s - successfully.",
                self.get_name(),
                self.__broker["host"],
            ...
            for mapping in self.__mapping:
                try:
                    ...
                    default_converters = {"bytes": "BytesMqttUplinkConverter"}

                    # Get converter class from "extension" parameter or default converter
                    converter_class_name = mapping["converter"].get(
                        "extension",
                        default_converters.get(mapping["converter"].get("type")),
                    )
                    if not converter_class_name:
                        self.__log.error(
                            "Converter type or extension class should be configured!"
                        )
                        continue
            ...
            for request in [
                entry for entry in self.__connect_requests if entry is not None
            ]:
                # requests are guaranteed to have topicFilter field. See __init__
                self.__subscribe(
                    request["topicFilter"], request.get("subscriptionQos", 1)
                )
            ...
            for request in [
                entry for entry in self.__disconnect_requests if entry is not None
            ]:
                # requests are guaranteed to have topicFilter field. See __init__
                self.__subscribe(
                    request["topicFilter"], request.get("subscriptionQos", 1)
            ...
            for request in [
                entry for entry in self.__attribute_requests if entry is not None
            ]:
                # requests are guaranteed to have topicFilter field. See __init__
                self.__subscribe(
                    request["topicFilter"], request.get("subscriptionQos", 1)
            ...
            result_codes = (
                RESULT_CODES_V5 if self._mqtt_version == 5 else RESULT_CODES_V3
            )
            if result_code in result_codes:
                self.__log.error(
                    "%s connection FAIL with error %s %s!",
                    self.get_name(),
                    result_code,
        ...
        try:
            if granted_qos[0] == 128:
                self.__log.error(
                    ...
                    self.get_name(),
                    self.__subscribes_sent.get(mid),
                    mid,
                )
            else:
                self.__log.info(
    ...
    def __threads_manager(self):
        ...
        elif number_of_needed_threads < len(self.__workers_thread_pool) and 1 < len(
            self.__workers_thread_pool
        ):
        ...
        log.info(
            f"#ConvertWorkers: {len(self.__workers_thread_pool)}, #Queued Messages: {self.__msg_queue.qsize()}"
        )

        self.__log.info(
            f"#ConvertWorkers: {len(self.__workers_thread_pool)}, #Queued Messages: {self.__msg_queue.qsize()}, Stats: {self.statistics}"
        )
    ...
    def _process_on_message(self):
        while not self.__stopped:
            if not self._on_message_queue.empty():
                    ...
                    for topic in topic_handlers:
                        ...
                        for converter in available_converters:
                            try:
                                ...
                                if (
                                    converter.config.get("sendDataOnlyOnChange", False)
                                    and self.__topic_content.get(message.topic)
                                    == content
                                ):
                ...
                topic_handlers = [
                    regex
                    for regex in self.__connect_requests_sub_topics
                    if fullmatch(regex, message.topic)
                ]
                ...
                if topic_handlers:
                    for topic in topic_handlers:
                        ...
                        if handler.get("deviceTypeTopicExpression"):
                            ...
                            found_device_type = (
                                device_type_match.group(0)
                                if device_type_match is not None
                                else handler["deviceTypeTopicExpression"]
                            )
                ...
                topic_handlers = [
                    regex
                    for regex in self.__disconnect_requests_sub_topics
                    if fullmatch(regex, message.topic)
                ]
                if topic_handlers:
                    for topic in topic_handlers:
                        ...
                        if found_device_name in self.__gateway.get_devices():
                            self.__log.info(
                                "Disconnecting device %s of type %s",
                                found_device_name,
                                found_device_type,
                            )
                ...
                topic_handlers = [
                    regex
                    for regex in self.__attribute_requests_sub_topics
                    if fullmatch(regex, message.topic)
                ]
                if topic_handlers:
                    try:
                        for topic in topic_handlers:
                            ...
                            if handler.get("attributeNameTopicExpression"):
                                ...
                                if attribute_name_match is not None:
                                    found_attribute_names = attribute_name_match.group(
                                        0
                                    )
                            elif handler.get("attributeNameJsonExpression"):
                                found_attribute_names = list(
                                    filter(
                            ...
                            self.__log.info(
                                "Will retrieve attribute %s of %s",
    ...
    def notify_attribute(
        self, incoming_data, attribute_name, topic_expression, value_expression, retain
    ):
        if (
            incoming_data.get("device") is None
            or incoming_data.get("value", incoming_data.get("values")) is None
        ):
            return
        ...
        topic = topic_expression.replace("${deviceName}", str(device_name)).replace(
            "${attributeKey}", str(attribute_name)
        )

        if len(attribute_name) <= 1:
            data = value_expression.replace(
                "${attributeKey}", str(attribute_name[0])
                ...
                if match(attribute_update["deviceNameFilter"], content["device"]):
                    for attribute_key in content["data"]:
                        if match(attribute_update["attributeFilter"], attribute_key):
                            ...
                            try:
                                data = (
                                    attribute_update["valueExpression"]
                                    .replace("${attributeKey}", str(attribute_key))
                                    .replace(
                                        "${attributeValue}",
                                        str(content["data"][attribute_key]),
                            ...
                            self._publish(
                                topic, data, attribute_update.get("retain", False)
                            )
                            self.__log.debug(
    ...
    def __process_rpc_request(self, content, rpc_config):
        ...
        if expects_response and defines_timeout:
            expected_response_topic = (
                rpc_config["responseTopicExpression"]
                .replace("${deviceName}", str(content["device"]))
            ...
            self.__subscribe(
                expected_response_topic, rpc_config.get("responseTopicQoS", 1)
            )
            ...
            self.__gateway.register_rpc_request_timeout(
                content, timeout, expected_response_topic, self.rpc_cancel_processing
            )
        ...
        request_topic: str = (
            rpc_config.get("requestTopicExpression")
            .replace("${deviceName}", str(content["device"]))
            .replace("${methodName}", str(content["data"]["method"]))
            .replace("${requestId}", str(content["data"]["id"]))
        )
        ...
        data_to_send_tags = TBUtility.get_values(
            rpc_config.get("valueExpression"), content["data"], "params", get_tag=True
        )
        ...
        for tag, value in zip(data_to_send_tags, data_to_send_values):
            data_to_send = data_to_send.replace(
                "${" + tag + "}", simplejson.dumps(value)
            )

        try:
            self.__log.info(
                "Publishing to: %s with data %s", request_topic, data_to_send
            )
    ...
    def server_side_rpc_handler(self, content):
        ...
        if rpc_method == "get" or rpc_method == "set":
            ...
            for rpc_config in self.__server_side_rpc:
                if (
                    search(rpc_config["deviceNameFilter"], content["device"])
            ...
            while not self.stopped:
                if not self.__msg_queue.empty():
                    ...
                    if converted_data and (
                        converted_data.get(ATTRIBUTES_PARAMETER)
                        or converted_data.get(TELEMETRY_PARAMETER)
                    ):

...
from thingsboard_gateway.tb_utility.tb_gateway_remote_configurator import (
    RemoteConfigurator,
)
...
try:
    ...
    from thingsboard_gateway.gateway.grpc_service.tb_grpc_manager import (
        TBGRPCServerManager,
    )
...
DEFAULT_CONNECTORS = {"mqtt": "MqttConnector"}

DEFAULT_STATISTIC = {"enable": True, "statsSendPeriodInSeconds": 3600}
DEFAULT_DEVICE_FILTER = {"enable": False}
...
class TBGatewayService:
    ...
    def __init__(self, config_file=None):
        ...
        self.__process_async_actions_thread = Thread(
            target=self.__process_async_device_actions,
            name="Async device actions processing thread",
            daemon=True,
        )
        ...
        try:
            logging.config.fileConfig(
                self._config_dir + "logs.conf", disable_existing_loggers=False
            )
            ...
            self.tb_client.client.send_telemetry(
                {
                    "ts": time() * 1000,
                    "values": {
                        "LOGS": "Logging loading exception, logs.conf is wrong: %s"
                        % (str(logging_error),)
                    },
                }
            )
        ...
        self._redis.start()
        self.__save_converted_data_thread = Thread(
            name="Save converted data to Memory event storage Thread",
            daemon=True,
            target=self.__send_to_storage,
        )
        self.__save_converted_data_thread.start()
        self._implemented_connectors = {}
        self._event_storage_types = {"memory": MemoryEventStorage}
        ...
        self.__rpc_scheduled_methods_functions = {
            "restart": {
                "function": execv,
                "arguments": (executable, [executable.split(pathsep)[-1]] + argv),
            },
            "reboot": {"function": system, "arguments": ("reboot 0",)},
        }
        self.__rpc_processing_thread = Thread(
            target=self.__send_rpc_reply_processing,
            daemon=True,
            name="RPC processing thread",
        )
        self.__rpc_processing_thread.start()
        self._event_storage = self._event_storage_types[
            self.__config["storage"]["type"]
        ...
        if (
            GRPC_LOADED
            and self.__grpc_config is not None
            and self.__grpc_config.get("enabled")
        ):
            self.__process_async_actions_thread.start()
            self.__grpc_manager = TBGRPCServerManager(self, self.__grpc_config)
            self.__grpc_manager.set_gateway_read_callbacks(
                self.__register_connector, self.__unregister_connector
            )
        self._load_connectors()
        self._connect_with_connectors()
        self.__load_persistent_devices()

        self.__devices_idle_checker = self.__config["thingsboard"].get(
            "checkingDeviceActivity", {}
        )
        self.__check_devices_idle = self.__devices_idle_checker.get(
            "checkDeviceInactivity", False
        )
        if self.__check_devices_idle:
            thread = Thread(
                name="Checking devices idle time",
                target=self.__check_devices_idle_time,
                daemon=True,
            )
            thread.start()
            log.info("Start checking devices idle time")

        self.__statistics = self.__config["thingsboard"].get(
        ...
        if self.__statistics["enable"]:
            self.__statistics_service = StatisticsService(
                self.__statistics["statsSendPeriodInSeconds"],
                self,
                log,
                config_path=self._config_dir + self.__statistics["configuration"]
        ...
        self.__min_pack_send_delay_ms = self.__config["thingsboard"].get(
        ...
        self.__min_pack_size_to_send = self.__config["thingsboard"].get(
            "minPackSizeToSend", 50
        )

        self.__data_transfer_threads: list[Thread] = []
        self.data_transfer_thread = Thread(
            target=self.read_data_from_storage,
            daemon=True,
            name="Send data to Thingsboard Main Thread",
        )
        self.data_transfer_thread.start()

        self.__device_filter_config = self.__config["thingsboard"].get(
            "deviceFiltering", DEFAULT_DEVICE_FILTER
        )
        self.__device_filter = None
        if self.__device_filter_config["enable"]:
            self.__device_filter = DeviceFilter(
                config_path=self._config_dir + self.__device_filter_config["filterFile"]
                if self.__device_filter_config.get("filterFile")
                else None
            )
        ...
        self._watchers_thread = Thread(
            target=self._watchers, name="Watchers", daemon=True
        )
    ...
    def _watchers(self):
        try:
            ...
            while not self.stopped:
                ...
                if (
                    ...
                    if self.__rpc_requests_in_progress:
                        for (
                            rpc_in_progress,
                            data,
                        ) in self.__rpc_requests_in_progress.items():
                            if cur_time >= data[1]:
                        ...
                        new_rpc_request_in_progress = {
                            key: value
                            for key, value in self.__rpc_requests_in_progress.items()
                            if value != "del"
                        }
                ...
                if (
                    not self.__request_config_after_connect
                    and self.tb_client.is_connected()
                ...
                if (
                    cur_time - gateway_statistic_send
                    > self.__statistics["statsSendPeriodInSeconds"] * 1000
                    and self.tb_client.is_connected()
                ):
                ...
                if (
                    cur_time - connectors_configuration_check_time
                    > self.__config["thingsboard"].get(
    ...
    def __init_remote_configuration(self, force=False):
        if (
            self.__config["thingsboard"].get("remoteConfiguration") or force
        ...
        try:
            ...
            if content is not None:
                ...
                if shared_attributes or client_attributes:
                    self.__process_attributes_response(
                        shared_attributes, client_attributes
                    )
                else:
    ...
    def __process_attributes_response(self, shared_attributes, client_attributes):
        self.__process_remote_logging_update(
            shared_attributes.get("RemoteLoggingLevel")
        )
    ...
    def __process_remote_logging_update(self, remote_logging_level):
        if remote_logging_level == "NONE":
            ...
            if (
                self.remote_handler.current_log_level != remote_logging_level
                or not self.remote_handler.activated
            ):
                self.main_handler.setLevel(remote_logging_level)
                self.remote_handler.activate(remote_logging_level)
                log.info(
                    "Remote logging has being updated. Current logging level is: %s ",
                    remote_logging_level,
                )

    def __process_deleted_gateway_devices(self, deleted_device_name: str):
        log.info(
            "Received deleted gateway device notification: %s", deleted_device_name
        )
        if deleted_device_name in list(self.__renamed_devices.values()):
            first_device_name = TBUtility.get_dict_key_by_value(
                self.__renamed_devices, deleted_device_name
            )
        ...
        if deleted_device_name in self.__connected_devices:
            ...
            log.debug(
                "Device %s - was removed from __connected_devices", deleted_device_name
            )
        if deleted_device_name in self.__saved_devices:
            del self.__saved_devices[deleted_device_name]
            log.debug(
                "Device %s - was removed from __saved_devices", deleted_device_name
            )
    ...
    def __process_renamed_gateway_devices(self, renamed_device: dict):
        if self.__config.get("handleDeviceRenaming", True):
            ...
            if old_device_name in list(self.__renamed_devices.values()):
                device_name_key = TBUtility.get_dict_key_by_value(
                    self.__renamed_devices, old_device_name
                )
            else:
            ...
            log.debug(
                "Received renamed device notification %r, but device renaming handle is disabled",
                renamed_device,
            )
    ...
    def request_device_attributes(
        self, device_name, shared_keys, client_keys, callback
    ):
        if client_keys is not None:
            self.tb_client.client.gw_request_client_attributes(
                device_name, client_keys, callback
            )
        if shared_keys is not None:
            self.tb_client.client.gw_request_shared_attributes(
                device_name, shared_keys, callback
            )
    ...
    def __register_connector(self, session_id, connector_key):
        if (
            self.__grpc_connectors.get(connector_key) is not None
            and self.__grpc_connectors[connector_key]["name"]
            not in self.available_connectors
        ):
            target_connector = self.__grpc_connectors.get(connector_key)
            connector = GrpcConnector(
                self, target_connector["config"], self.__grpc_manager, session_id
            )
            connector.setName(target_connector["name"])
            self.available_connectors[connector.get_name()] = connector
            self.__grpc_manager.registration_finished(
                Status.SUCCESS, session_id, target_connector
            )
            log.info(
                "GRPC connector with key %s registered with name %s",
                connector_key,
                connector.get_name(),
            )
            ...
            log.error(
                "GRPC connector with key: %s - already registered!", connector_key
            )
        else:
            self.__grpc_manager.registration_finished(
                Status.NOT_FOUND, session_id, None
            )
            log.error(
                "GRPC configuration for connector with key: %s - not found",
                connector_key,
            )
    ...
    def __unregister_connector(self, session_id, connector_key):
        if (
            self.__grpc_connectors.get(connector_key) is not None
            and self.__grpc_connectors[connector_key]["name"]
            in self.available_connectors
        ):
            connector_name = self.__grpc_connectors[connector_key]["name"]
            target_connector: GrpcConnector = self.available_connectors.pop(
                connector_name
            )
            self.__grpc_manager.unregister(Status.SUCCESS, session_id, target_connector)
            log.info(
                "GRPC connector with key %s and name %s - unregistered",
                connector_key,
            ...
            log.error(
                "GRPC configuration for connector with key: %s - not found in configuration and not registered",
                connector_key,
            )
    ...
    def _generate_persistent_key(connector, connectors_persistent_keys):
        if (
            ...
            and connectors_persistent_keys.get(connector["name"]) is not None
        ):
            ...
            for connector in self.__config["connectors"]:
                try:
                    ...
                    if connector["type"] == "grpc" and self.__grpc_manager is None:
                        log.error(
                            "Cannot load connector with name: %s and type grpc. GRPC server is disabled!",
                            connector["name"],
                        )
                        continue
                    ...
                    if connector["type"] != "grpc":
                        ...
                        if connector.get("useGRPC", True):
                            ...
                            connector_class = TBModuleLoader.import_module(
                                connector["type"], module_name
                            )

                        if (
                            self.__grpc_manager
                            and self.__grpc_manager.is_alive()
                            and connector_class
                        ):
                            connector_persistent_key = self._generate_persistent_key(
                                connector, connectors_persistent_keys
                            )
                        else:
                            connector_class = TBModuleLoader.import_module(
                                connector["type"],
                        ...
                        if connector_class is None:
                            log.warning(
                                "Connector implementation not found for %s",
                                connector["name"],
                            )
                        else:
                            self._implemented_connectors[
                        ...
                        if connector.get("key") == "auto":
                            self._generate_persistent_key(
                                connector, connectors_persistent_keys
                            )
                        else:
                            connector_persistent_key = connector["key"]
                        log.info(
                            "Connector key for GRPC connector with name [%s] is: [%s]",
                            connector["name"],
                    ...
                    try:
                        ...
                        log.debug(e)
                        log.warning(
                            "Cannot parse connector configuration as a JSON, it will be passed as a string."
                        )
        ...
        for connector_type in self.connectors_configs:
            for connector_config in self.connectors_configs[connector_type]:
                if self._implemented_connectors.get(connector_type.lower()) is not None:
                    if (
                        connector_type.lower() != "grpc"
                        and "Grpc"
                        not in self._implemented_connectors[
                        ...
                        for config in connector_config["config"]:
                            ...
                            try:
                                if connector_config["config"][config] is not None:
                                    connector: Thread = self._implemented_connectors[
                                        connector_type
                                    ](
                                        self,
                        ...
                        self.__grpc_connectors.update(
                            {connector_config["grpc_key"]: connector_config}
                        )
                        if connector_type.lower() != "grpc":
                            connector_dir_abs = "/".join(
                                self._config_dir.split("/")[:-2]
                            )
                            ...
                            thread = Thread(
                                ...
                                daemon=True,
                                name="Separate DRPC Connector",
                            )
                            thread.start()

    def _run_connector(self, connector_abs_path, connector_config_json):
        subprocess.run(
            ["python3", connector_abs_path, connector_config_json, self._config_dir],
            check=True,
    ...
    def check_connector_configuration_updates(self):
        ...
        for connector_type in self.connectors_configs:
            for connector_config in self.connectors_configs[connector_type]:
                if (
                    stat(connector_config["config_file_path"])
                    != connector_config["config_updated"]
                ):
    ...
    def send_to_storage(self, connector_name, data) -> Status:
        ...
        try:
            ...
            if self.__device_filter:
                device_valid = self.__device_filter.validate_device(
                    connector_name, data
                )
            ...
            if filtered_data:
                self.__converted_data_queue.put(
                    (connector_name, filtered_data), True, 100
                )
                    ...
                    for data in data_array:
                        if not connector_name == self.name:
                            ...
                            if not TBUtility.validate_converted_data(data):
                                log.error(
                                    "Data from %s connector is invalid.", connector_name
                                )
                                continue
                            if data.get("deviceType") is None:
                                device_name = data["deviceName"]
                                if (
                                    self.__connected_devices.get(device_name)
                                    is not None
                                ):
                                    data["deviceType"] = self.__connected_devices[
                                        device_name
                                    ]["device_type"]
                                elif self.__saved_devices.get(device_name) is not None:
                                    data["deviceType"] = self.__saved_devices[
                                        device_name
                                    ]["device_type"]
                                else:
                                    data["deviceType"] = "default"
                            if (
                                data["deviceName"] not in self.get_devices()
                                and self.tb_client.is_connected()
                            ):
                                self.add_device(
                                    data["deviceName"],
                                    {
                                        "connector": self.available_connectors[
                                            connector_name
                                        ]
                                    },
                        ...
                        if self.__check_devices_idle:
                            self.__connected_devices[data["deviceName"]][
                                "last_receiving_data"
                            ] = time()

                        data = self.__convert_telemetry_to_ts(data)

                        max_data_size = self.__config["thingsboard"].get(
                            "maxPayloadSizeBytes", 400
                        )
                        if self.__get_data_size(data) >= max_data_size:
                            ...
                            for attribute in data["attributes"]:
                                ...
                                if adopted_data_size >= max_data_size:
                                    ...
                                    self.__send_data_pack_to_storage(
                                        adopted_data, connector_name
                                    )
                            ...
                            for ts_kv_list in telemetry:
                                ...
                                for kv in ts_kv_list["values"]:
                                    if ts in ts_to_index:
                                        ...
                                        adopted_data["telemetry"][ts_to_index[ts]][
                                            "values"
                                        ].update(kv_data)
                                    else:
                                        kv_data = {
                                            "ts": ts,
                                            "values": {kv: ts_kv_list["values"][kv]},
                                        }
                                        adopted_data["telemetry"].append(kv_data)
                                        ts_to_index[ts] = (
                                            len(adopted_data["telemetry"]) - 1
                                        )
                            ...
                            if (
                                len(adopted_data["telemetry"]) > 0
                                or len(adopted_data["attributes"]) > 0
                            ):
                                self.__send_data_pack_to_storage(
                                    adopted_data, connector_name
                                )
    ...
    def check_size(self, devices_data_in_event_pack):
        if self.__get_data_size(devices_data_in_event_pack) >= self.__config[
            "thingsboard"
        ].get("maxPayloadSizeBytes", 400):
    ...
    def read_data_from_storage(self):
        ...
        for i in range(5):
            data_transfer_threads.append(
                Thread(
                    target=self.__read_data_from_storage,
                    daemon=True,
        ...
        while not self.stopped:
            try:
                if self.tb_client.is_connected():
                    ...
                    if (
                        self.__remote_configurator is None
                    ...
                    if events:
                        for event in events:
                            ...
                            if not devices_data_in_event_pack.get(
                            ...
                            if current_event.get("telemetry"):
                                if isinstance(current_event["telemetry"], list):
                                    ...
                                    devices_data_in_event_pack[
                                        current_event["deviceName"]
                                    ]["telemetry"].append(current_event["telemetry"])
                        ...
                        if self.tb_client.is_connected() and (
                            ...
                            while not self._published_events.empty():
                                if (
                                    (
                                        self.__remote_configurator is not None
                                        and self.__remote_configurator.in_process
                                    )
                                    or not self.tb_client.is_connected()
                                    or self._published_events.empty()
                                ...
                                for _ in range(
                                    min(
                                        self.__min_pack_size_to_send,
                                        self._published_events.qsize(),
                                    )
                                ):
                                    event: TBPublishInfo = self._published_events.get(
                                        True, 10
                                    )
                                    events.append(event)
                                ...
                                for event in events:
                                    try:
                                        if self.tb_client.is_connected() and (
                                            ...
                                            if (
                                                self.tb_client.client.quality_of_service
                                                == 1
                                            ):
                                                result_code = event.get()
                                                success = (
                                                    result_code == event.TB_ERR_SUCCESS
                                                )
                                            else:
                                                success = True
                                        else:
                ...
                log.error(
                    ">>>> Thingsboard Client Connection Error!! Check Thingsboard!! <<<<"
                )
                log.exception(e)
                sleep(2)
    ...
    def __send_data(self, devices_data_in_event_pack):
        try:
            for device in devices_data_in_event_pack:
                final_device_name = (
                    device
                    if self.__renamed_devices.get(device) is None
                    else self.__renamed_devices[device]
                )
                if devices_data_in_event_pack[device].get("attributes"):
                    if device == self.name or device == "currentThingsBoardGateway":
                        self._published_events.put(
                            self.tb_client.client.send_attributes(
                                devices_data_in_event_pack[device]["attributes"]
                            )
                        )
                    else:
                        self._published_events.put(
                            self.tb_client.client.gw_send_attributes(
                                final_device_name,
                                devices_data_in_event_pack[device]["attributes"],
                            )
                        )
                if devices_data_in_event_pack[device].get("telemetry"):
                    if device == self.name or device == "currentThingsBoardGateway":
                        ...
                        log.debug(
                            f"Device {device}'s Result Condition: {publish_info.get()}"
                        )
                        self._published_events.put(publish_info)
        ...
        try:
            ...
            if device is not None:
                ...
                if connector_name is not None:
                    ...
                    log.error(
                        "Received RPC request but connector for the device %s not found. Request data: \n %s",
                        content["device"],
                        dumps(content),
                    )
            else:
                try:
                    ...
                    if module is not None:
                        ...
                        if self.connectors_configs.get(module):
                            log.debug(
                                'Connector "%s" for RPC request "%s" found',
                                module,
                                content["method"],
                            )
                            for connector_name in self.available_connectors:
                                if (
                                    ...
                                    log.debug(
                                        "Sending command RPC %s to connector %s",
                                        content["method"],
                                        connector_name,
                                    )
                                    result = self.available_connectors[
                                        connector_name
                                    ].server_side_rpc_handler(content)
                        elif (
                            module == "gateway"
                            or module in self.__remote_shell.shell_commands
                        ):
                            result = self.__rpc_gateway_processing(request_id, content)
                        else:
                            log.error('Connector "%s" not found', module)
                            result = {
                                "error": "%s - connector not found in available connectors."
                                % module,
                                "code": 404,
                            }
                        if result is None:
                    ...
                    self.send_rpc_reply(
                        None, request_id, '{"error":"%s", "code": 500}' % str(e)
                    )
                    log.exception(e)
        except Exception as e:
            log.exception(e)
    ...
    def __rpc_gateway_processing(self, request_id, content):
        log.info(
            "Received RPC request to the gateway, id: %s, method: %s",
            str(request_id),
            content["method"],
        )
        ...
        if self.__remote_shell is not None:
            method_function = self.__remote_shell.shell_commands.get(
                method_to_call, self.__gateway_rpc_methods.get(method_to_call)
            )
        else:
            log.info("Remote shell is disabled.")
            method_function = self.__gateway_rpc_methods.get(method_to_call)
        if (
            method_function is None
            and method_to_call in self.__rpc_scheduled_methods_functions
        ):
            seconds_to_restart = (
                arguments * 1000 if arguments and arguments != "{}" else 0
            )
            self.__scheduled_rpc_calls.append(
                [
                    time() * 1000 + seconds_to_restart,
                    self.__rpc_scheduled_methods_functions[method_to_call],
                ]
            )
            log.info(
                "Gateway %s scheduled in %i seconds",
                method_to_call,
    ...
    def send_rpc_reply(
        ...
        self.__rpc_processing_queue.put(
            (
                device,
                req_id,
                content,
    ...
    def __send_rpc_reply(
        ...
        try:
            ...
            if device is not None and success_sent is not None:
                ...
                self.tb_client.client.gw_send_rpc_reply(
                    device, req_id, content, quality_of_service=quality_of_service
                )
            elif device is None and success_sent is not None:
                self.tb_client.client.send_rpc_reply(
                    req_id,
                    dumps(rpc_response),
                    quality_of_service=quality_of_service,
    ...
    def register_rpc_request_timeout(self, content, timeout, topic, cancel_method):
        ...
        self.__rpc_register_queue.put(
            {"topic": topic, "data": (content, timeout, cancel_method)}, False
        )

    def cancel_rpc_request(self, rpc_request):
        content = self.__rpc_requests_in_progress[rpc_request][0]
        self.send_rpc_reply(
            device=content["device"], req_id=content["data"]["id"], success_sent=False
        )
    ...
    def _attribute_update_callback(self, content, *args):
        ...
        if content.get("device") is not None:
            try:
                self.__connected_devices[content["device"]][
                    "connector"
    ...
    def update_device(self, device_name, event, content):
        if (
            event == "connector"
            and self.__connected_devices[device_name].get(event) != content
        ):
            self.__save_persistent_devices()
    ...
    def get_devices(self, connector_name: str = None):
        return (
            ...
            if connector_name is None
                ...
                device_name: self.__connected_devices[device_name]["device_type"]
                for device_name in self.__connected_devices.keys()
                if self.__connected_devices[device_name].get("connector") is not None
                and self.__connected_devices[device_name]["connector"].get_name()
                == connector_name
            }
        )
        ...
        while not self.stopped:
            if not self.__async_device_actions_queue.empty():
                ...
                if action == DeviceActions.CONNECT:
                    self.add_device(
                        data["deviceName"],
                        {CONNECTOR_PARAMETER: self.available_connectors[data["name"]]},
                        data.get("deviceType"),
                    )
    ...
    def __load_persistent_connector_keys(self):
        persistent_keys = {}
        if (
            PERSISTENT_GRPC_CONNECTORS_KEY_FILENAME in listdir(self._config_dir)
            and path.getsize(self._config_dir + PERSISTENT_GRPC_CONNECTORS_KEY_FILENAME)
            > 0
        ):
            try:
                persistent_keys = load_file(
                    self._config_dir + PERSISTENT_GRPC_CONNECTORS_KEY_FILENAME
                )
            except Exception as e:
                log.exception(e)
    ...
    def __save_persistent_keys(self, persistent_keys):
        try:
            with open(
                self._config_dir + PERSISTENT_GRPC_CONNECTORS_KEY_FILENAME, "w"
            ) as persistent_keys_file:
                persistent_keys_file.write(
                    dumps(persistent_keys, indent=2, sort_keys=True)
                )
        except Exception as e:
            log.exception(e)
    ...
    def __load_persistent_devices(self):
        devices = None
        if (
            CONNECTED_DEVICES_FILENAME in listdir(self._config_dir)
            and path.getsize(self._config_dir + CONNECTED_DEVICES_FILENAME) > 0
        ):
            try:
        ...
        if devices is not None:
            ...
            for device_name in devices:
                try:
                    if not isinstance(devices[device_name], list):
                        open(self._config_dir + CONNECTED_DEVICES_FILENAME, "w").close()
                        log.debug(
                            "Old connected_devices file, new file will be created"
                        )
                        return
                    if self.available_connectors.get(devices[device_name][0]):
                        device_data_to_save = {
                            "connector": self.available_connectors[
                                devices[device_name][0]
                            ],
                            "device_type": devices[device_name][1],
                        }
                        if (
                            len(devices[device_name]) > 2
                            and device_name not in self.__renamed_devices
                        ):
            ...
            self.__connected_devices = (
                {} if self.__connected_devices is None else self.__connected_devices
            )
    ...
    def __save_persistent_devices(self):
        with self.__lock:
            ...
            for device in self.__connected_devices:
                if self.__connected_devices[device]["connector"] is not None:
                    data_to_save[device] = [
                        self.__connected_devices[device]["connector"].get_name(),
            ...
            with open(
                self._config_dir + CONNECTED_DEVICES_FILENAME, "w"
            ) as config_file:
                try:
    ...
    def __check_devices_idle_time(self):
        check_devices_idle_every_sec = self.__devices_idle_checker.get(
            "inactivityCheckPeriodSeconds", 1
        )
        disconnect_device_after_idle = self.__devices_idle_checker.get(
        ...
        while not self.stopped:
            ...
            for device_name in for_deleting:
                ...
                log.debug(
                    "Delete device %s for the reason of idle time > %s.",
                    device_name,

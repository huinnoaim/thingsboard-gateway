...
class AlarmManager:
    ...
    def get_alarm_rule(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Failed to get alarm rule. Status code: {response.status_code}"
                )
    ...
    def get_exam_with_serial_number(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Failed to exam with serial. Status code: {response.status_code}"
                )
        except requests.exceptions.RequestException as e:
            logger.error(f"Error during API call: {e}")
    ...
    def upsert_alarm(self, topic, payload):
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(
                    f"Failed to upsert alarm. Status code: {response.status_code}"
                )
        except requests.exceptions.RequestException as e:
            logger.error(f"Error during API call: {e}")
    ...
    def upsert_alarm_rule(self, payload):
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(
                    f"Failed to upsert alarm rule. Status code: {response.status_code}"
                )
            self.get_alarm_rule()
    ...
    def check_alarm(self, serial_number, sensor_type, value):
        ...
        if sensor_type == "hr":
            self.check_hr(
                condition["hrLimit"],
                int(value),
                exam_info["examination_id"],
                exam_info["ward_id"],
                exam_info["hospital_id"],
            )

    def check_hr(self, hr_alarm_limit, value, exam_id, ward_id, hospital_id):
        if (
            hr_alarm_limit["RED"]["HIGH"] is not None
            and value > hr_alarm_limit["RED"]["HIGH"]
        ):
            self.send_mqtt_alarm(
                AlarmStatus.UNACK.value,
                exam_id,
                "HR HIGH",
                AlarmSeverity.CRITICAL.value,
                f"{value} > {hr_alarm_limit['RED']['HIGH']}",
                value,
                ward_id,
                hospital_id,
            )
        elif (
            hr_alarm_limit["RED"]["LOW"] is not None
            and value < hr_alarm_limit["RED"]["LOW"]
        ):
            self.send_mqtt_alarm(
                AlarmStatus.UNACK.value,
                exam_id,
                "HR LOW",
                AlarmSeverity.CRITICAL.value,
                f"{value} < {hr_alarm_limit['RED']['LOW']}",
                value,
                ward_id,
                hospital_id,
            )
        elif (
            hr_alarm_limit["YELLOW"]["HIGH"] is not None
            and value > hr_alarm_limit["YELLOW"]["HIGH"]
        ):
            self.send_mqtt_alarm(
                AlarmStatus.UNACK.value,
                exam_id,
                "HR HIGH",
                AlarmSeverity.MAJOR.value,
                f"{value} > {hr_alarm_limit['YELLOW']['HIGH']}",
                value,
                ward_id,
                hospital_id,
            )
        elif (
            hr_alarm_limit["YELLOW"]["LOW"] is not None
            and value < hr_alarm_limit["YELLOW"]["LOW"]
        ):
            self.send_mqtt_alarm(
                AlarmStatus.UNACK.value,
                exam_id,
                "HR LOW",
                AlarmSeverity.MAJOR.value,
                f"{value} < {hr_alarm_limit['YELLOW']['LOW']}",
                value,
                ward_id,
                hospital_id,
            )
            ...
            if last_alarm != None:
                ...
                self.client.pub(
                    f"alarms/{hospital_id}/{ward_id}/{exam_id}", json.dumps(last_alarm)
                )
                self.remove_last_alarm(last_alarm.get("id"))
            return

    def send_mqtt_alarm(
        self, status, examId, type, severity, limits, value, ward_id=1, hospital_id=123
    ):
        ...
        if last_alarm == None:
            ...
            self.client.pub(
                f"alarms/{hospital_id}/{ward_id}/{examId}", json.dumps(payload)
            )
        elif (
            last_alarm
            and last_alarm.get("type") == type
            and last_alarm.get("severity") == severity
        ):
            return
        else:
            last_alarm["status"] = AlarmStatus.CLEARED.value
            self.client.pub(
                f"alarms/{hospital_id}/{ward_id}/{examId}", json.dumps(last_alarm)
            )
            self.remove_last_alarm(last_alarm.get("id"))
